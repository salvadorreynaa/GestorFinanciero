...
def crear_recordatorio():
    try:
        ...
        fecha_recordatorio = data.get('fecha')
        descripcion = data.get('descripcion')
        estado = 'pendiente'

        if not all([movimiento_id, fecha_recordatorio, descripcion]):
        ...
        cur.execute("SELECT id FROM recordatorios WHERE movimiento_id = %s AND estado != 'completado'", (movimiento_id,))
        ...
        if recordatorio_existente:
            ...
            cur.execute("""
                ...
                SET fecha_recordatorio = %s, descripcion = %s,
                    estado = %s, created_at = CURRENT_TIMESTAMP 
                WHERE movimiento_id = %s
                RETURNING id
            """, (fecha_recordatorio, descripcion, estado, movimiento_id))
        else:
            # Crear nuevo recordatorio
            cur.execute("""
                INSERT INTO recordatorios (movimiento_id, fecha_recordatorio, descripcion, estado) 
                VALUES (%s, %s, %s, %s) 
                RETURNING id
            """, (movimiento_id, fecha_recordatorio, descripcion, estado))
...
def obtener_recordatorio(movimiento_id):
    try:
        ...
        cur.execute("""
            SELECT id, fecha_recordatorio, descripcion, estado
            FROM recordatorios
            WHERE movimiento_id = %s AND estado != 'completado'
        ...
        if recordatorio:
            return jsonify({
                'id': recordatorio[0],
                'fecha_recordatorio': recordatorio[1].strftime('%Y-%m-%d') if recordatorio[1] else None,
                'descripcion': recordatorio[2],
                'estado': recordatorio[3]
            }), 200
        else:
            return jsonify({'estado': 'no_existe'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def eliminar_recordatorio(movimiento_id):
    try:
        ...
        cur.execute("UPDATE recordatorios SET estado = 'completado' WHERE movimiento_id = %s", (movimiento_id,))
        conn.commit()
